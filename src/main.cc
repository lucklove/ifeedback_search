...
int main()
{
    ...
    CROW_ROUTE(app, "/search").methods("POST"_method)([&pdb, &wdb](const crow::request& req)
    {
        ...
        auto victors = searcher.search(x["keyword"].s(), subject_id, type_id);
        ...
        for(size_t i = offset; i < std::min(victors.size(), offset + limit ? offset + limit : victors.size()); ++i)
        {
            ...
            res["victors"][i-offset]["score"] = victors[i].second;
            res["victors"][i-offset]["content"] = pdb.getContent(victors[i].first);
        }
        return crow::response{res}; 
    });
